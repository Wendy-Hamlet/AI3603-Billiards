...
\begin{enumerate}
    ...
    \item \textbf{High precision requirements}: A $1^\circ$ error in $\phi$ can mean the difference between pocketing and missing. The reward landscape is discontinuous.
...
\begin{itemize}
    ...
    \item $\phi \in [0^\circ, 360^\circ]$: encoded as $(\sin\phi, \cos\phi)$ to handle wraparound
    \item $\theta \in [0^\circ, 90^\circ]$: normalized to $[0, 1]$
...
\begin{itemize}
    \item 72 bins ($5^\circ$ resolution)
...
\begin{itemize}
    ...
    \item Y-mirror: $x \rightarrow 1-x$, $\phi \rightarrow 180^\circ-\phi$
    \item $180^\circ$ rotation: $x \rightarrow 1-x$, $y \rightarrow 1-y$, $\phi \rightarrow \phi+180^\circ$
\end{itemize}
...
Consider a state where either Ball 3 or Ball 7 could be pocketed. The MCTS expert might choose Ball 3 in some games and Ball 7 in others, depending on subtle factors or randomness in its search. These correspond to completely different $\phi$ angles (e.g., $45^\circ$ vs $220^\circ$).

When training with MSE loss, the network learns to \textit{average} these actions:
\begin{equation}
\hat{\phi} = \frac{\phi_{Ball3} + \phi_{Ball7}}{2} \approx 132.5^\circ
\end{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Angle accuracy is critical}: With 72 bins ($5^\circ$ each), 16\% accuracy means expected error of $\sim$15 bins = $75^\circ$, far too large for precise pocketing.
