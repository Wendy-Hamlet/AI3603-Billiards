...
I designed a 64-dimensional state encoding with semantic grouping to capture all game-relevant information while maintaining a fixed input size regardless of how many balls remain on the table.

The state vector consists of six components. First, the \textbf{cue ball} is represented by 2 dimensions encoding its position $(x, y)$ normalized to $[0, 1]$. Second, \textbf{own balls} occupy 21 dimensions representing 7 balls with 3 features each (position and pocketed status), sorted by distance to the cue ball to provide implicit priority ordering. Third, \textbf{opponent balls} follow the same 21-dimensional structure. Fourth, the \textbf{8-ball} receives special 3-dimensional treatment due to its game-ending significance (position and pocketed flag). Fifth, \textbf{pocket positions} contribute 12 dimensions encoding the $(x, y)$ coordinates of all 6 pockets, providing explicit target locations for shot selection. Finally, \textbf{global information} uses 5 dimensions to encode remaining ball counts for both sides, turn indicator, and game phase (whether targeting the 8-ball).
...
For consistency in state representation, balls not present in simplified scenarios are marked as ``already pocketed'' in the state encoding, maintaining the 64-dimensional input throughout all stages.
...
The parallel infrastructure deployed 100 worker processes across a multi-core server, achieving a collection rate of approximately 1,800 games per hour (based on measurements from long\_mcts games, accounting for data collection overhead). Each game generates multiple state-action pairs (one per expert decision), and over several weeks of continuous collection, the pipeline accumulated a dataset of \textbf{1,787,130 state-action pairs}. This represents one of the largest billiards imitation learning datasets reported in the literature.
...
The initial approach used feedforward networks with Transformer-based architectures, varying capacity to understand the impact of model size. The \textbf{small} configuration used 4 hidden layers with 256 units each, totaling approximately 8 million parameters. The \textbf{large} configuration used 8 Transformer blocks with 512 hidden dimensions and 8 attention heads, reaching 26 million parameters. Both used GELU activations, layer normalization, and dropout for regularization.
...
The 360-degree range is discretized into 36 bins of 10 degrees each. The network outputs a 36-dimensional softmax distribution, and training uses cross-entropy loss for $\phi$ while maintaining MSE loss for other action components. At inference time, the predicted angle is the center of the highest-probability bin. This approach avoids the ``averaging'' problem to some extent: the network can learn a multi-modal distribution over bins rather than being forced to output a single point estimate.
...
The discrete classification approach with 36 bins achieved 50.2\% accuracy on the $\phi$ prediction task (where random guessing would yield $\sim$2.8\% for 36 bins). This represents substantial improvement over random chance, translating to an expected angular error of approximately 18 degrees. However, this precision is still insufficient for reliable pocketing, which typically requires sub-degree accuracy. The model reached its best validation loss (3.76) at epoch 60, demonstrating stable training convergence.
...
Second, \textbf{angular precision requirements are extreme for billiards}. With 36 bins spanning 360 degrees, each bin covers 10 degrees. The Discrete 36 model achieved 50.2\% accuracy, translating to an expected angular error of approximately 18 degrees. While this is substantially better than regression-based approaches, successful pocketing typically requires sub-degree precision. The Equivariant network with 72 bins (5 degrees per bin) achieved only 16\% accuracy, demonstrating that finer discretization alone does not solve the problem. Achieving sufficient angular accuracy through supervised learning would require either much finer discretization with significantly more data or a fundamentally different approach.
...
long\_mcts & 96.9\% & 80.6\% \\
\textbf{long\_opening} & \textbf{96.2\%} & \textbf{79.5\%} \\
\hline
\end{tabular}
\end{center}
...
Among MCTS variants, \textbf{long\_opening} represents the final optimized solution, incorporating an opening safety strategy for unbroken racks. This variant achieves 96.2\% win rate against BasicAgent and 79.5\% against BasicAgentPro, demonstrating that strategic opening play provides additional robustness while maintaining strong overall performance. The performance is comparable to the base \textbf{long\_mcts} (96.9\% vs Basic, 80.6\% vs Pro), with a slight trade-off that prioritizes safe opening play.
...
\subsubsection*{SAC State Encoding (64 dimensions)}
