...
The results in Table~\ref{tab:robustness} were surprising: even against this nearly-passive opponent, both baseline agents lose approximately 20\% of games. Since \texttt{RandomAgent} almost never pockets balls intentionally, these losses must occur when the baseline agent defeats itself.
...
The \textbf{pocketing reward} uses proportional value to encourage clearing balls efficiently. When the agent pockets its own ball, the reward scales with progress: $r_{pocket} = \frac{n_{pocketed}}{n_{remaining}} \times 100$. This design gives higher rewards for pocketing balls when fewer remain, reflecting their increased strategic importance. Pocketing opponent balls incurs a penalty of $-50$.
...
The \textbf{defense reward} is computed retroactively: when the opponent fails on their turn (misses or fouls), the agent receives $+5$ added to its previous action's reward, encouraging defensive positioning.
...
\subsubsection{Discovery of the ``Lazy'' Strategy}

Training on the 4-ball scenario with self-play revealed an unexpected and frustrating phenomenon. Rather than learning to pocket balls aggressively, the agent converged to what I call the ``lazy'' or ``stalling'' strategy.
...
\begin{table}[h]
\caption{Self-Play with Large Rewards ($\pm$1000): Convergence to Lazy Strategy}
\label{tab:exp1}
\begin{center}
...
\subsubsection{Reward Modifications to Counter Lazy Strategy}

I experimented with several reward modifications to discourage the lazy strategy.
...
\begin{table}[h]
\caption{Compact Rewards ($\pm$100): Same Lazy Convergence}
\label{tab:exp2}
\begin{center}
\begin{tabular}{|r|c|c|c|c|}
\hline
...
\begin{table}[h]
\caption{Strict Draw Penalty: Reduced Lazy Behavior}
\label{tab:exp3}
\begin{center}
\begin{tabular}{|r|c|c|c|c|}
\hline
...
Encouraged by success in the 4-ball scenario, I attempted to train on the 6-ball configuration. The results were disheartening: the agent immediately reverted to the lazy strategy, despite the strict draw penalty that had worked for 4 balls.
...
Given the fundamental challenges encountered with reinforcement learning, I pivoted to imitation learning: directly learning from the strong MCTS agent developed by my teammate. The hypothesis was that by observing expert behavior, a neural network could learn to approximate the MCTS policy without requiring explicit reward signals or extensive exploration.
...
The imitation learning agent achieved only 35.5\% win rate against BasicAgent, substantially lower than the 94.4\% achieved by the MCTS expert it was trained to imitate. This dramatic performance gap directly illustrates the multi-modal action problem: the learned policy predicts ``averaged'' actions that correspond to neither of the expert's valid shot choices, resulting in frequent misses.

MCTS succeeds where learning methods fail for several fundamental reasons. First, MCTS provides \textbf{explicit look-ahead evaluation}: by simulating action consequences before execution, MCTS avoids the 20\% ``self-defeat'' rate observed in greedy baselines (Section II-B). The agent can reject shots that risk pocketing the 8-ball prematurely or scratching, something that RL struggled to learn implicitly.
...
Pivoting to self-play with a ball count curriculum showed promise in the simplified 4-ball scenario. However, the agent discovered a ``lazy'' equilibrium strategy: avoiding all pocketing attempts to run out the 60-shot clock. Strict draw penalties successfully countered this behavior for 4 balls, reducing draw rates from 51\% to 36\%. But scaling to 6 balls immediately caused regression to lazy play, despite extensive reward function modifications.
...
\textbf{Opponent robustness analysis}: I designed and executed experiments using a RandomAgent to probe the robustness of baseline opponents. This analysis revealed that BasicAgent and BasicAgentPro lose approximately 20\% of games through self-defeat (premature 8-ball pocketing, scratches), even against a nearly-passive opponent. This insight motivated the exploration of look-ahead methods.
...
\textbf{Final solution}: The long\_mcts agent achieved 94.4\% win rate against BasicAgent and 80.6\% against BasicAgentPro, representing our team's final submission.
