...
\begin{enumerate}
    \item \textbf{Physics-based agent}: A baseline using geometric calculations (Section II)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{PHYSICS-BASED BASELINE}
...
\textbf{Terminal rewards}: $+1000$ (win), $-1000$ (loss).
...
Despite extensive engineering, SAC training failed to produce a competent agent. The final commit message ``RL is horrifying'' (commit \texttt{3008e3b}) summarizes months of debugging.

\textbf{Root causes identified}:

\begin{enumerate}
    \item \textbf{Sparse, delayed rewards}: Meaningful feedback only occurs after complete physics simulation. The agent receives no gradient signal for ``almost good'' shots.
    
    \item \textbf{High precision requirements}: A 1° error in $\phi$ angle can mean the difference between pocketing and missing entirely. The reward landscape is essentially discontinuous.
    
    \item \textbf{Exploration inefficiency}: Random Gaussian exploration in 5D continuous space almost never produces successful pockets, providing no learning signal during early training.
    
    \item \textbf{Credit assignment}: With 20+ shots per game and delayed terminal rewards, attributing success to specific actions is extremely difficult.
\end{enumerate}

\textbf{Lesson learned}: For tasks requiring high-precision continuous control with binary outcomes, end-to-end RL may require prohibitive sample complexity. The availability of a fast simulator makes search-based methods far more practical.
...
\begin{itemize}
    \item \textbf{Look-ahead}: Evaluates consequences of actions through simulation
    \item \textbf{No training}: Works directly from physics engine
    \item \textbf{Handles multi-modality}: Naturally explores multiple shot options
    \item \textbf{Robustness}: Can test actions with noise perturbations
\end{itemize}
...
\begin{itemize}
    \item Physics-based agent baseline (60\% vs Basic)
    \item Complete SAC reinforcement learning framework with parallel training
    \item Imitation learning pipeline: data collection (1.78M samples), 5 network architectures, training infrastructure
    \item Parallel evaluation framework for agent comparison
    \item Analysis of failure modes and lessons learned
\end{itemize}
