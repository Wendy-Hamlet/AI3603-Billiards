...
- collect_ball_states: 收集球状?
- save_balls_state / restore_balls_state: 状态保?恢复
...
class PoolEnv():
    ...
    def __init__(self, verbose=False):
        ...
        Args:
            verbose: 是否打印详细的击球信息（训练时建议False?
        """
        # 桌面和球
        self.table = None
        self.balls = None
        ...
        # 记录所有shot，用于赛后render正常比赛，或者保存比赛记?
        self.shot_record = pt.MultiSystem()
        
        # 击球参数噪声标准差（模拟真实误差）（前期调试的时候可以先禁用）（0.1-0.1-0.1-0.003-0.003这个组合就显著让agent的性能退?从单局平均25杆到了单局平均35杆）
        self.noise_std = {
            'V0': 0.1,      # 速度标准?
            'phi': 0.1,      # 水平角度标准差（度）
            'theta': 0.1,    # 垂直角度标准差（度）
            'a': 0.003,       # 横向偏移标准?球半径的比例（无量纲?
            'b': 0.003        # 纵向偏移标准?球半径的比例（无量纲?
        }
    ...
    def get_observation(self, player=None):
        ...
        返回值：
            tuple: (balls, my_targets, table)
                ...
                balls (dict): 球状态字典，{ball_id: Ball对象}
                    ...
                    Ball 对象属性：
                        ball.state.rvw: np.ndarray, shape=(3,3)
                            [0]: position, np.array([x, y, z])  # 位置，单位：?
                            [1]: velocity, np.array([vx, vy, vz])  # 速度，单位：??
                            [2]: spin, np.array([wx, wy, wz])  # 角速度，单位：弧度/?
                ...
                table (Table): 球桌对象
                    属性：
                        table.w: float  # 球桌宽度，单位：米（?.99米）
                        table.l: float  # 球桌长度，单位：米（?.98米）
    ...
    def get_done(self,):
        ...
        返回：tuple
            (True, {'winner': 'A'/'B'/'SAME', 'hit_count': int})  # 已结?
            (False, {})  # 未结?
        """
        if self.done:
    ...
    def reset(self, state=None, target_ball:str=None):
        ...
        参数?
            ...
            target_ball: Player A 目标球型
                'solid': A打实?1-7), B打条?9-15)
                'stripe': A打条?9-15), B打实?1-7)
        """
        # 目前不支持恢复到指定state，只能恢复到新开一局的状?
    ...
    def take_shot(self, action:dict):
        ...
        if "8" in new_pocketed:
            ...
            if len(remaining_own) == 0:
                if self.verbose:
                    print(f"🏆 Player {player} 成功打进?，获胜！")
                self.winner = self.players[self.curr_player]
            else:
                if self.verbose:
                    print(f"💥 Player {player} 误打?（自身球未清空），判负！")
                    print(f"🏆 Player {self.players[1 - self.curr_player]} 获胜?)
                self.winner = self.players[1 - self.curr_player]
            self.done = True
        ...
        if first_contact_ball_id is not None:
            ...
            if len(remaining_own_before) > 0:
                ...
                if first_contact_ball_id != '8':
                    if self.verbose:
                        print(f"⚠️ Player {player} 己方球已清空但首次碰撞非?，交换球权?)
                    self.curr_player = 1 - self.curr_player
                self.last_state = save_balls_state(self.balls)
                self.hit_count += 1
                if self.hit_count >= self.MAX_HIT_COUNT:
        ...
        # 判断是否打进自己球，确定下一个击球方
        if own_pocketed:
            print(f"🎯 Player {player} 打进?{own_pocketed}，继续出杆?)
        else:
