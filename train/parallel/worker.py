...
def worker_process(worker_id: int,
    ...
    try:
        ...
        from train.environment.state_encoder import ActionSpace
        ...
        print(f"[Worker {worker_id}] Started, own_balls={own_balls}, enemy_balls={enemy_balls}")
        ...
        while not stop_event.is_set():
            ...
            try:
                while not policy_queue.empty():
                    ...
                    # print(f"[Worker {worker_id}] Policy updated")
            except Exception as e:
                pass
            
            # 运行一局游戏
            try:
                experiences = run_game(env, actor, action_space)
                games_played += 1
                
                # 计算本局总奖励
                game_reward = sum(exp['reward'] for exp in experiences)
                ...
                print(f"[Worker {worker_id}] Error in game: {e}")
                traceback.print_exc()
                continue
        
        print(f"[Worker {worker_id}] Stopped, played {games_played} games")
...
def run_game(env, actor, action_space) -> List[Dict]:
    """
    运行一局完整的自对弈游戏
    ...
    # 收集所有经验
    experiences = env.get_all_experiences()
