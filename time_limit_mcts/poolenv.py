...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 观看当前杆，使用ESC退出
        # show.show(env.shot_record[-1], title=f"hit count: {env.hit_count}")
        ...
        if done:
            ...
            # for i in range(len(env.shot_record)):
                # show.show(env.shot_record[i], title=f"hit count: {i}")
            
            ## 观看整个过程 使用 p 和 n 控制 上一杆/ 下一杆
            # show.show(env.shot_record, title=f"all record")
            break
